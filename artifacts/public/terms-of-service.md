# Terms of Service — Learning Copilot

**Last Updated:** December 30, 2024

---

## 1. About the Service

The **Learning Copilot** is an AI-powered educational assistant developed by **Aisystant**. It helps adult learners navigate their personal development journey within the Aisystant educational ecosystem.

The Service operates as a ChatGPT Action that connects to the Aisystant MCP server to provide personalized learning guidance.

---

## 2. Acceptance of Terms

By using the Learning Copilot, you agree to these Terms of Service. If you do not agree, please do not use the Service.

---

## 3. Description of Service

The Learning Copilot provides:

| Feature | Description |
|---------|-------------|
| **Learning Stage Assessment** | Helps identify your current stage as a learner |
| **Personalized Guidance** | Provides recommendations for your learning path |
| **Progress Tracking** | Tracks weekly learning sessions and habits |
| **Learning Support** | Helps overcome learning blocks and maintain motivation |

---

## 4. User Responsibilities

By using this Service, you agree to:

- Provide accurate information about your learning goals and progress
- Use the Service for legitimate educational purposes only
- Not attempt to exploit, hack, or reverse-engineer the Service
- Not use the Service for any illegal or harmful activities
- Not impersonate others or misrepresent your identity

---

## 5. Prohibited Uses

The following uses are **strictly prohibited**:

- Using the Service to generate spam, malware, or harmful content
- Attempting to extract proprietary prompts or system instructions
- Using automated systems (bots) to interact with the Service at scale
- Reselling or redistributing access to the Service
- Using the Service to harass, abuse, or harm others
- Circumventing usage limits or access controls

---

## 6. Disclaimer of Warranties

The Learning Copilot is provided **"AS IS"** without warranties of any kind.

**Important limitations:**

- The Service is an AI-based educational tool, not a replacement for professional advice
- AI responses may occasionally be inaccurate or incomplete
- The Service does not guarantee specific learning outcomes
- The assistant cannot make life decisions on your behalf

---

## 7. Limitation of Liability

To the maximum extent permitted by law:

- Aisystant shall not be liable for any indirect, incidental, or consequential damages
- Our total liability is limited to the amount you paid for the Service (if any)
- We are not responsible for decisions you make based on the assistant's recommendations

---

## 8. Intellectual Property

- The Learning Copilot, including its design, prompts, and methodology, is owned by Aisystant
- User-generated content (your messages) remains your property
- By using the Service, you grant us a license to process your inputs for providing responses

---

## 9. Third-Party Services

The Learning Copilot integrates with:

| Service | Purpose |
|---------|---------|
| **OpenAI ChatGPT** | AI conversation platform |
| **Aisystant Platform** | Learning management and user data |
| **Aisystant Identity** | OAuth authentication |

Your use of these services is also governed by their respective terms.

---

## 10. Account Termination

We may suspend or terminate your access to the Service if you:

- Violate these Terms of Service
- Engage in prohibited uses
- Attempt to abuse or exploit the Service

---

## 11. Modifications to Terms

We reserve the right to modify these Terms at any time. Continued use of the Service after changes constitutes acceptance of the new Terms.

---

## 12. Governing Law

These Terms are governed by the laws of the Russian Federation. Disputes shall be resolved in the courts of Moscow, Russia.

---

## 13. Contact Information

For questions about these Terms:

- **Email:** info@system-school.ru
- **Website:** https://aisystant.com
- **Telegram:** @aisystant_platform

---

## 14. Links

- [Privacy Policy](./privacy-policy.md)
- [Aisystant Platform](https://aisystant.com)
- [GitHub Repository](https://github.com/aisystant/fsm-mcp)

---

**Aisystant**
Operator: Workshop for Engineer-Managers
https://aisystant.com
