...
Линейный поток из 9 узлов:

```
┌──────────────────┐
│     Webhook      │  POST /check
│                  │  Принимает запрос с ответом стажёра
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│ Code in JavaScript│  Валидация входных данных
│                  │  Проверяет 4 обязательных поля
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│   HTTP Request   │  MCP: get_guide_sections
│                  │  Получает список секций курса
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│Code in JavaScript1│  Поиск секции по названию
│                  │  Находит section_url по section_name
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│  HTTP Request1   │  MCP: get_section_content
│                  │  Получает текст секции (норматив)
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│Code in JavaScript2│  Формирование промпта
│                  │  Собирает: вопрос + ответ + норматив
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│  HTTP Request2   │  Claude API
│                  │  Отправляет промпт в claude-3-haiku
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│Code in JavaScript3│  Форматирование ответа
│                  │  Преобразует JSON LLM в комментарий
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│Respond to Webhook│  Возврат результата
│                  │  Отправляет ответ вызывающей системе
└──────────────────┘
```
...
```
Webhook ──▶ Валидация ──▶ MCP:sections ──▶ Поиск секции ──▶ MCP:content
                                                                 │
                                                                 ▼
Respond ◀── Форматирование ◀── Claude API ◀── Формирование промпта
```
