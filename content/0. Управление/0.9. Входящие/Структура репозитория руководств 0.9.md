...
```
guide-folder/                      ← РУКОВОДСТВО (папка)
├── index.md                       ← Оглавление + метаданные руководства
├── 01-section-name/               ← РАЗДЕЛ (подпапка)
│   ├── 01-subsection-name.md      ← ПОДРАЗДЕЛ (документ)
│   ├── 02-subsection-name.md      ← ПОДРАЗДЕЛ (документ)
│   ├── 03-sub-subsection/         ← ПОДРАЗДЕЛ второго уровня (подпапка)
│   │   ├── 01-topic.md            ← ПОДПОДРАЗДЕЛ (документ)
│   │   └── 02-topic.md            ← ПОДПОДРАЗДЕЛ (документ)
│   └── assets/                    ← Медиа для раздела
├── 02-section-name/               ← РАЗДЕЛ (подпапка)
│   └── ...
├── assets/                        ← Общие медиа руководства
└── _check/                        ← Материалы для ДЗ-чекера
```

### 1.4 Типы материалов в руководствах
...
|-----------|-------------|
| **Личное развитие** | Системное саморазвитие, Практики саморазвития, Введение в системное мышление |
| **Рабочее развитие** | Системный фитнес, Рациональная работа, Системное мышление |
| **Исследовательское развитие** | Методология, Системная инженерия, Инженерия личности |
| **Менеджмент** | Системный менеджмент, Интеллект-стек |
| **Универсальное** | Сводный справочник понятий (формируется из кратких материалов) |
...
│   │   ├── program.md
│   │   ├── roadmap.md
│   │   ├── 2.1-fitness/                   ← РУКОВОДСТВО: Системный фитнес
│   │   ├── 2.2-rational-work/             ← РУКОВОДСТВО: Рациональная работа
│   │   └── 2.3-systems-thinking/          ← РУКОВОДСТВО: Системное мышление
│   │
│   ├── 3-research/                        ← Исследовательское развитие
│   │   ├── program.md
│   │   ├── roadmap.md
│   │   ├── 3.1-methodology/               ← РУКОВОДСТВО: Методология
│   │   ├── 3.2-systems-engineering/       ← РУКОВОДСТВО: Системная инженерия
│   │   └── 3.3-personality-engineering/   ← РУКОВОДСТВО: Инженерия личности
│   │
│   └── 4-management/                      ← Менеджмент
│       ├── program.md
│       ├── roadmap.md
│       ├── 4.1-systems-management/        ← РУКОВОДСТВО: Системный менеджмент
│       └── 4.2-intellect-stack/           ← РУКОВОДСТВО: Интеллект-стек
│
├── universal/                             ← УНИВЕРСАЛЬНОЕ РУКОВОДСТВО
│   ├── index.md                           ← Сводный справочник
│   ├── concepts/                          ← Понятия (краткие материалы)
│   │   ├── by-guide/                      ← Понятия по руководствам
│   │   │   ├── self-development.md
│   │   │   ├── systems-thinking.md
│   │   │   └── ...
│   │   └── by-topic/                      ← Понятия по темам
│   │       ├── roles.md
│   │       ├── systems.md
│   │       └── ...
│   └── ideas/                             ← Ключевые идеи
...
```json
{
  ...
  "programs": [
    ...
    },
    {
      ...
      "guides": [
        {"code": "2.1", "slug": "fitness", "title": "Системный фитнес"},
        {"code": "2.2", "slug": "rational-work", "title": "Рациональная работа"},
        {"code": "2.3", "slug": "systems-thinking", "title": "Системное мышление"}
      ]
    },
    {
      "id": "research",
      ...
      "guides": [
        {"code": "3.1", "slug": "methodology", "title": "Методология"},
        {"code": "3.2", "slug": "systems-engineering", "title": "Системная инженерия"},
        {"code": "3.3", "slug": "personality-engineering", "title": "Инженерия личности"}
      ]
    },
    {
      "id": "management",
      "code": "C.Program.Management",
      "title": "Менеджмент",
      "path": "programs/4-management",
      "guides": [
        {"code": "4.1", "slug": "systems-management", "title": "Системный менеджмент"},
        {"code": "4.2", "slug": "intellect-stack", "title": "Интеллект-стек"}
      ]
    }
  ],
  "universal": {
    "path": "universal",
    "description": "Сводный справочник понятий и идей"
  },
  "meta": {
...
Примеры:
1.1.01.02    = Личное развитие → Саморазвитие → Раздел 1 → Подраздел 2
1.1.01.03.01 = Личное развитие → Саморазвитие → Раздел 1 → Подраздел 3 → Подподраздел 1
2.3.05.01    = Рабочее развитие → Системное мышление → Раздел 5 → Подраздел 1
4.1.02.03    = Менеджмент → Системный менеджмент → Раздел 2 → Подраздел 3
```
...
| Руководство | `{P}.{G}-{slug}/` | `1.1-self-development/` |
| Раздел (папка) | `{SS}-{slug}/` | `01-attention/` |
| Подраздел (документ) | `{SS}-{slug}.md` | `02-focus.md` |
| Подраздел (папка) | `{SS}-{slug}/` | `03-techniques/` |
| Подподраздел (документ) | `{SS}-{slug}.md` | `01-timeboxing.md` |
| Картинка | `fig-{SS}-{name}.png` | `fig-03-matrix.png` |
| Таблица | `table-{SS}-{name}.csv` | `table-01-types.csv` |
| ДЗ | `hw-{SS}.md` | `hw-03.md` |
